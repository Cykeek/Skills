...
|---|---|---|
| Quick copy edit / typo fix | Direct | Run `lint_content.py --fix` directly without full 5-phase brief definition. |
| Single headline / CTA generation | Abbreviated | Skip brief creation, apply `style-guide.md` principles and generate options immediately. |
| Full blog / case study / whitepaper | Full Loop | Run `generate_brief.py` first to enforce the 4 clarifying questions and build outline. |
...
|---|---|---|
| `productivity/resume-doctor` | When writing or tailoring a resume, CV, or LinkedIn profile. | For general copywriting, articles, or marketing pages. |
| `design/designer-god` | When creating visual layouts, wireframes, or UI components alongside copy. | For pure text generation or blog writing. |
| `business/cal-com-api` | When setting up scheduling workflow copy or booking page messaging. | For general content creation not tied to booking API flows. |

---

## Quality Loop

Before delivering final copy:
- [ ] Run `lint_content.py` to verify zero AI tells, formal density ≤ 50%, and no em dashes.
- [ ] Check headings against `seo-playbook.md` format (if search-focused).
- [ ] Verify that tone strictly matches the target audience specified in `brief.json`.

**Confidence tag:** 🟢 High / 🟡 Medium / 🔴 Low - append to the summary output.
