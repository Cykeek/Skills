...
```
OLD: .agents/skills/content-writer/
NEW: .agents/skills/content/content-writer/
```
...
Located in `.agents/skills/scripts/`:
...
jobs:
  validate:
    ...
    steps:
      ...
      - run: python .agents/skills/scripts/validate_all.py
      - run: python .agents/skills/scripts/test_integration.py --all
  ...
  audit:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: python .agents/skills/scripts/audit_changed.py
```
...
Dashboard: `.agents/skills/health/dashboard.json` (updated by CI)
