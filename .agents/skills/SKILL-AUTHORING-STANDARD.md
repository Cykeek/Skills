...
│   └── *
└── .agents-plugin/       # Optional: marketplace plugin
    └── plugin.json
```
...
- [ ] .agents-plugin/plugin.json (if publishing)
...
### Automation
- [ ] `.agents-plugin/plugin.json` valid (if applicable)
...
- `.agents-plugin/` — Claude Code plugin
