...
def find_skills(skills_root: Path) -> List[Path]:
    """Find all skill directories (supports both direct and domain/skill structure)."""
    skills = []
    for entry in skills_root.iterdir():
        if not entry.is_dir() or entry.name.startswith('.') or entry.name in {"templates", "standards"}:
            continue
        if (entry / "SKILL.md").exists():
            skills.append(entry)
        else:
            # Check domain subdirectories (two levels deep)
            for subentry in entry.iterdir():
                if subentry.is_dir() and not subentry.name.startswith('.') and (subentry / "SKILL.md").exists():
                    skills.append(subentry)
...
def main():
    ...
    parser.add_argument("--skills-root", default=".agents/skills", help="Skills root directory")
