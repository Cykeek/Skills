...
### Validate a Skill
```bash
python .agents/scripts/validate_skill.py .agents/skills/content/content-writer/
```

### Validate All Skills (JSON output for CI)
```bash
python .agents/scripts/validate_all.py --json
```

### Scaffold New Skill (Interactive)
```bash
python .agents/scripts/scaffold_skill.py --interactive
```

### Scaffold from Spec File
```bash
python .agents/scripts/scaffold_skill.py .agents/skills/templates/example-api-contract-tester-spec.json
```

### Dry Run (Preview)
```bash
python .agents/scripts/scaffold_skill.py .agents/skills/templates/example-seo-optimizer-spec.json --dry-run
```
...
- Auto-discovers skills in `.agents/skills/<domain>/<skill>/`
...
- name: Validate Skills
  run: |
    python .agents/scripts/validate_all.py --json --strict > validation.json
...
- repo: local
  hooks:
    - id: validate-skills
      name: Validate Skills
      entry: python .agents/scripts/validate_all.py --fail-fast
      language: system
