...
def extract_summary(text: str) -> str:
    match = re.search(r'(?:Professional Summary|Summary|Profile|Objective)\s*\n(.*?)(?:\n\s*(?:Skills|Core Competencies|Work Experience|Experience|Education|Projects|Certifications)\s*\n)', text, re.DOTALL | re.IGNORECASE)
    if match:
...
def extract_skills(text: str) -> list[str]:
    skills = []
    match = re.search(r'(?:Skills|Core Competencies|Technical Skills|Key Skills|Areas of Expertise)\s*\n(.*?)(?:\n\s*(?:Work Experience|Experience|Education|Projects|Certifications)\s*\n)', text, re.DOTALL | re.IGNORECASE)
    if match:
...
def extract_experience(text: str) -> list[dict]:
    experience = []
    match = re.search(r'(?:Work Experience|Experience|Professional Experience|Employment History)\s*\n(.*?)(?:\n\s*(?:Education|Projects|Certifications|Skills)\s*\n)', text, re.DOTALL | re.IGNORECASE)
    if not match:
        return experience
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m ats_parsers.taleo <text_file>")
        sys.exit(1)
