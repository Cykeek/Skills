...
def extract_skills(text: str) -> list[str]:
    skills = []
    match = re.search(r'(?:Skills|Core Competencies|Technical Skills|Key Skills)\s*\n(.*?)(?:\n\s*(?:Work Experience|Experience|Education|Projects|Certifications)\s*\n)', text, re.DOTALL | re.IGNORECASE)
    if match:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m ats_parsers.icims <text_file>")
        sys.exit(1)
