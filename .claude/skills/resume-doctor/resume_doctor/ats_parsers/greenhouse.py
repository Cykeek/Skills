"""
Greenhouse ATS Parser Simulation
"""
import re
...
def parse(text: str) -> ParsedResume:
    """Simulate Greenhouse parsing from pdftotext -layout output"""
...
def extract_experience(text: str) -> list[dict]:
    experience = []
    # Pattern: Company + Role + Dates + Bullets
    sections = re.split(r'\n\s*(?:Company|Work Experience|Experience)\s*\n', text, flags=re.IGNORECASE)
    if len(sections) < 2:
        return experience

    exp_text = sections[1]
    # Split by date patterns or company names
    # This is a simplified parser - real ATS uses ML
    ...
    for entry in entries:
        ...
        # Try to separate company and role
...
def extract_dates(text: str) -> list[str]:
    """Extract date ranges in MM/YYYY format"""
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python -m ats_parsers.greenhouse <text_file>")
        sys.exit(1)
